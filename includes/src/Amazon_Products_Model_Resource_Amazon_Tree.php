 ...
 */
class Amazon_Products_Model_Resource_Amazon_Tree extends Varien_Data_Tree_Dbp
{
     ...
     *
     * @var Amazon_Products_Model_Resource_Amazon_Collection
     */
    ...
    protected $_inactiveAmazonIds  = null;
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            $resource->getConnection('amazon_products_write'),
            $resource->getTableName('amazon_products/amazon'),
            array(
     ...
     *
     * @access public
     * @param Amazon_Products_Model_Resource_Amazon_Collection $collection
     * @return Amazon_Products_Model_Resource_Amazon_Tree
     */
     ...
     *
     * @access public
     * @param unknown_type $amazon
     * @param unknown_type $newParent
     * @param unknown_type $prevNode
     * @author Ultimate Module Creator
     */
    public function move($amazon, $newParent, $prevNode = null)
    {
        Mage::getResourceSingleton('amazon_products/amazon')
            ->move($amazon->getId(), $newParent->getId());
        parent::move($amazon, $newParent, $prevNode);
        $this->_afterMove($amazon, $newParent, $prevNode);
    }
     ...
     * @return Amazon_Products_Model_Resource_Amazon_Tree
     */
    protected function _afterMove($amazon, $newParent, $prevNode)
    {
        Mage::app()->cleanCache(array(Amazon_Products_Model_Amazon::CACHE_TAG));
        return $this;
    }
    ...
    protected function _createCollectionDataSelect($sorted = true)
    {
        ...
        $amazonsTable = Mage::getSingleton('core/resource')
            ->getTableName('amazon_products/amazon');
        $subConcat = $this->_conn->getConcatSql(array('main_table.path', $this->_conn->quote('/%')));
        $subSelect = $this->_conn->select()
            ->from(array('see' => $amazonsTable), null)
     ...
     * @author Ultimate Module Creator
     */
    public function getExistingAmazonIdsBySpecifiedIds($ids)
    {
        if (empty($ids)) {
            return array();
        }
    ...
    )
    {
        ...
        if ($toLoad) {
            ...
            foreach ($collection as $amazon) {
                if ($this->getNodeById($amazon->getId())) {
                    $this->getNodeById($amazon->getId())->addData($amazon->getData());
                }
            }
            foreach ($this->getNodes() as $node) {
     ...
     * @author Ultimate Module Creator
     */
    public function addInactiveAmazonIds($ids)
    {
        if (!is_array($this->_inactiveAmazonIds)) {
            $this->_initInactiveAmazonIds();
        }
        $this->_inactiveAmazonIds = array_merge($ids, $this->_inactiveAmazonIds);
        return $this;
    }
     ...
     * @author Ultimate Module Creator
     */
    protected function _initInactiveAmazonIds()
    {
        $this->_inactiveAmazonIds = array();
        return $this;
    }
    /**
     * Retrieve inactive amazon products ids
     ...
     * @author Ultimate Module Creator
     */
    public function getInactiveAmazonIds()
    {
        if (!is_array($this->_inactiveAmazonIds)) {
            $this->_initInactiveAmazonIds();
        }
        return $this->_inactiveAmazonIds;
    }
    ...
    {
        $this->_inactiveItems = $this->getInactiveAmazonIds();
    ...
    protected function _getInactiveItemIds($collection)
    {
        ...
        $table = Mage::getSingleton('core/resource')->getTable('amazon_products/amazon');
        $bind = array(
            'cond' => 0,
        );
        $select = $this->_conn->select()
